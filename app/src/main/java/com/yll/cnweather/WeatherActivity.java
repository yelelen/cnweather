...
public class WeatherActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        bgImageView = (ImageView)findViewById(R.id.bing_pic_img);
        nav_home = (Button)findViewById(R.id.nav_button);
        ...
        String bgImageUrl = prefs.getString("bg_image", null);
        if (bgImageUrl != null)
            Glide.with(this).load(bgImageUrl).into(bgImageView);
        else
            loadBgImage();
    ...
    private void loadBgImage(){
        final String bgImageUrl = "http://guolin.tech/api/bing_pic";
        HttpUtil.sendOkhttpRequest(bgImageUrl, new Callback() {
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        Toast.makeText(WeatherActivity.this,
                                "背景图片加载失败", Toast.LENGTH_SHORT).show();
                    }
                });
            }
            ...
            @Override
            public void onResponse(Call call, Response response) throws IOException {
                final String strBgImage = response.body().string();
                SharedPreferences.Editor editor = PreferenceManager.getDefaultSharedPreferences(WeatherActivity.this).edit();
                editor.putString("bg_image", strBgImage);
                editor.apply();
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Glide.with(WeatherActivity.this).load(strBgImage).into(bgImageView);
                    }
                });
            }
        });
